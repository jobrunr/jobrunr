...
class TaskStatisticsTest {
    ...
    @BeforeEach
    void setUp() {
        statistics = new TaskStatistics(dashboardNotificationManager);
    }

    @Test
    void ifOneRunTookTooLongNoNotificationIsShown() {
        statistics.logRun(2, true, pollInterval, now().minusSeconds(15), now());
    ...
    void ifRunFailsOnceThenSucceedsAgainexceptionCounterIsZero() {
        ...
        statistics.logRun(1, true, pollInterval, now().minusSeconds(1), now());
