...
public class BackgroundJobRequest {
     ...
     * <pre>{@code
     *      BackgroundJobRequest.schedule(CarbonAwarePeriod.between(Instant.now(), Instant.now().plus(5, ChronoUnit.HOURS)), new MyJobRequest());
     * }</pre>
     *
     ...
     * <h5>An Example with {@code CarbonAwarePeriod}:</h5>
     * <pre>{@code
     *      BackgroundJobRequest.schedule(id, CarbonAwarePeriod.between(Instant.now(), Instant.now().plus(5, ChronoUnit.HOURS)), new MyJobRequest());
     * }</pre>
     *
     * @param id         the uuid with which to save the job
     * @param scheduleAt the moment in time at which the job will be enqueued.
