...
public class JobDocumentMapper {
    ...
    public Document toInsertDocument(Job job) {
        ...
        if (job.hasState(StateName.AWAITING)) {
            document.put(Jobs.CARBON_AWARE_DEADLINE, toMicroSeconds(job.<CarbonAwareAwaitingState>getJobState().getTo()));
        }
    ...
    public Document toUpdateDocument(Job job) {
        ...
        if (job.hasState(StateName.AWAITING)) {
            document.put(Jobs.CARBON_AWARE_DEADLINE, toMicroSeconds(((CarbonAwareAwaitingState) job.getJobState()).getTo()));
        }
        job.getRecurringJobId().ifPresent(recurringJobId -> document.put(Jobs.FIELD_RECURRING_JOB_ID, recurringJobId));
