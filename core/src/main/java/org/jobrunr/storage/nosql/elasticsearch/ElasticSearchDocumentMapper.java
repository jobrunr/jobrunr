...
public class ElasticSearchDocumentMapper {
    ...
    public Map<Object, Object> toMap(BackgroundJobServerStatus serverStatus) {
        ...
        map.put(FIELD_DELETE_DELETED_JOBS_AFTER, serverStatus.getPermanentlyDeleteDeletedJobsAfter().toString());
        map.put(FIELD_FIRST_HEARTBEAT, serverStatus.getFirstHeartbeat().toEpochMilli());
    ...
    public BackgroundJobServerStatus toBackgroundJobServerStatus(Map<Object, Object> map) {
        return new BackgroundJobServerStatus(
                ...
                autobox(map.get(FIELD_NAME), String.class),
    ...
    public Job toJob(final GetResponse<Map> response) {
        final JsonData jsonData = response.fields().get(Jobs.FIELD_JOB_AS_JSON);
        final String jobAsJson = ((JsonString) jsonData.toJson().asJsonArray().get(0)).getString();
        return jobMapper.deserializeJob(jobAsJson);
    }

    public Job toJob(final Hit<Map> hit) {
